...
Large Language Models, or LLMs, since the full name is a bit of a tongue
twister, are much more present in our day-to-day life than we would think,
or even anticipated just a few years back. But what are they truly? What do
they define, and what is their use? \cite{llm_intro_placeholder}

\subsubsection{What Is a Large Language Model?}

A Large Language Model is, to put it simply, a mathematical model that
generates text --- like filling in the gap for the next word in a sentence,
much like the ``word suggestions'' that appear atop your keyboard when you
are typing a sentence on your phone. However, do not let that simple
comparison fool you. While your phone might struggle to predict anything
beyond your usual coffee order, an LLM operates on a scale that is frankly
dizzying. Imagine taking every book, article, and scrap of code on the
internet and feeding it into a digital brain that maps out the intricate
relationships between every word and concept. \cite{llm_scale_placeholder}

By calculating the probability of what comes next based on billions of
parameters, these models move past simple ``autofill'' and into the realm
of reasoning and creativity. They are not just guessing letters; they are
grasping the nuances of human intent. At the heart of this process is a
structure called a \emph{Transformer}. Think of it as a system that allows
the model to look at a whole paragraph at once, rather than one word at a
time. This \emph{attention} mechanism ensures it remembers the beginning of
your story while it is writing the end, maintaining a coherent thread that
feels remarkably human.

\subsubsection{What Can LLMs Do?}

Because they are masters of pattern recognition, their utility stretches
far beyond answering trivia or writing emails:

\begin{itemize}
  \item \textbf{Creative Partners:} They can brainstorm marketing slogans
    or draft poetry in the style of a 19th-century novelist.
  \item \textbf{Technical Aides:} They translate complex programming
    languages as easily as they translate French into English.
  \item \textbf{Educational Assistants:} They can explain intricate
    concepts, answer questions, and adapt their tone to the level of the
    reader --- which is precisely the use case explored in this project.
\end{itemize}

This versatility, however, comes with important caveats, which we explore
...
\subsubsection{Bias Considerations: Feedback Loop Degradation via RLHF}

A subtler but significant limitation concerns the feedback mechanism used to
improve LLMs \cite{rlhf_challenges_medium}. Techniques such as
\emph{Reinforcement Learning from Human Feedback} (RLHF) and Constitutional
AI are used to align model behavior with human preferences and reduce harmful
outputs. This system relies on users rating model responses positively or
negatively, with the model updated accordingly \cite{ouyang2022rlhf}. While
this process is intended to align model behavior with user expectations, it
can produce perverse effects when users consistently submit poorly formulated
prompts.

When a user provides an ambiguous, incomplete, or incoherent query, the model
has no reliable basis on which to produce a relevant answer. It then generates
a response that is statistically plausible given the input, but which
...
leading to a measurable degradation in general response quality over time.
A study by Chen et al. \cite{chen2023chatgpt} empirically documented this
...
% Inclure les références pour la partie 1.1 et 1.2 (partie de lisa) + corriger les bugs de références
